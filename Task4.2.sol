...
contract Ballot {
    ...
    constructor(bytes32[] memory proposalNames, uint256 votingDurationMinutes) {
        ...
        registeredVoters[chairperson].weight = 1;
    ...
    modifier onlyChairperson() {
        require(msg.sender == chairperson, "Only the chairperson can call this.");
        _;
    }
    ...
    function authorizeVoter(address voter) external onlyChairperson {
    ...
    function setVoterWeight(address voter, uint256 weight) external onlyChairperson withinVotingPeriod {
        require(!registeredVoters[voter].hasVoted, "Voter has already voted.");
        require(weight > 0, "Weight must be greater than zero.");

        registeredVoters[voter].weight = weight;
    }
